...
import com.hexiaofei.springeurekaclient.service.SpringContextHolder;
...
public class RedisUtil<T extends Serializable> {
    ...
    private static JedisPool jedisReader =(JedisPool) SpringContextHolder.getBean("jedisReader");
    private static JedisPool jedisWriter =(JedisPool) SpringContextHolder.getBean("jedisWriter");
    ...
    public Long decrLong(String key,int expireTime) {
        Jedis writer = null;
        try {
            key = tagName + key;
            writer = jedisWriter.getResource();
